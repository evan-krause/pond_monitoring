...
cleaned_path <- paste0("data/lp_cleaned.rds")

if (file.exists(cleaned_path)) {
  pond_dat <- readRDS(cleaned_path) #load previously cleaned data
} else {
  source('src/lp_clean.R') #if no cached data; run cleaning script (prompts for source file)
}
# heatmap functions ----
...
plot_profile_heatmap <- function(param, station_id, data = pond_dat) {
  data <- data |>
...
heatmap_out_dir <- "outputs/heatmaps_2"
